...
def grid_maker():
    ...
    grid_input = input('''
        ...
        Note: If you want to try different grids, and you have
        a hard time to create one using this input, then please
        wait for the next update 'Pathfinder for Real Dummies'.                          
        >>> 
            ''')
...
def print_path(robot_name, path_number, path):
    '''
    This function was created for dummies. It converts
    Geometry coordinates into plain text directions.
    '''
    path_str = f"\t{robot_name}'s Path #{path_number}: [A, " # The beginning
    for i in range(1, len(path)):
        dx = path[i][0] - path[i - 1][0] # the difference between the previous point and the current point x-axis
        dy = path[i][1] - path[i - 1][1] # the difference between the previous point and the current point y-axis
        direction = DIRECTIONS.get((dx, dy), "unknown") # match the difference with DIRECTIONS dictionary. Set 'unknown' if it doesn't exist (Won't happen though!).
        path_str += direction + ", "
    path_str += "B]" # The end
    print(GREEN + path_str + RESET)
...
while status:
    ...
    if all_paths:
        ...
        for index, path in enumerate(all_paths):
            print_path(robot_name,index + 1, path)
            print("\n" + RESET)
    else:
